...
7. Always assign the document's reporting period (for example `2026Q1` or
   `2026Q2`) to trailing statistics: `metrics.sharpe_36m` and
   `metrics.max_drawdown_bps`. Never leave `period` null on those fields.
8. Always extract `fund.share_class` from the fund header or the terms block.
